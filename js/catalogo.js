...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (cat) {
    // Las fotos y la cinta de marcas se cargan después y mueven la página,
    // así que el salto se repite cuando ya todo tiene su alto definitivo.
    const irAlCatalogo = () => {
      const destino = $("#catalogo");
      if (destino) destino.scrollIntoView({ behavior: "auto", block: "start" });
    };
    irAlCatalogo();
    requestAnimationFrame(irAlCatalogo);
    setTimeout(irAlCatalogo, 250);
    window.addEventListener("load", () => setTimeout(irAlCatalogo, 120));
  }
});
