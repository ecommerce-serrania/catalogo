...
const ICONOS = {
  ...
  mudo:  '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M4 9v6h4l5 4V5L8 9H4z"/><path d="M16.5 8.5l5 5m0-5l-5 5" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round"/></svg>',
  audio: '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M4 9v6h4l5 4V5L8 9H4z"/><path d="M16.5 9a4 4 0 010 6M19 6.5a7.5 7.5 0 010 11" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round"/></svg>'
};

/* ---------- Rutas de imagen ----------
   Las fotos pueden estar en .jpg o en .webp: el sitio prueba los dos.
   El orden lo define CONFIG.formatoFotos en config.js. */
function formatos(){
  const primero = (window.CONFIG && window.CONFIG.formatoFotos) || "jpg";
  return primero === "webp" ? ["webp", "jpg", "jpeg", "png"] : ["jpg", "jpeg", "webp", "png"];
}

function rutaFoto(id, n){ return "img/productos/" + id + "-" + n + "." + formatos()[0]; }
...
/* Imagen que va probando extensiones y, si ninguna existe, deja un reemplazo */
function imagenFlexible(base, alt, clase, respaldoClase, respaldoTexto, exts){
  const lista = exts || formatos();
  return '<img src="' + base + '.' + lista[0] + '" alt="' + escapar(alt) + '"'
       + (clase ? ' class="' + clase + '"' : '')
       + ' data-base="' + base + '" data-exts="' + lista.join(",") + '" data-i="0"'
       + ' data-rclase="' + respaldoClase + '" data-rtexto="' + escapar(respaldoTexto) + '"'
       + ' loading="lazy" decoding="async" onerror="siguienteFormato(this)">';
}

/* Si una extensión falla, prueba la siguiente; agotadas todas, pone el reemplazo */
function siguienteFormato(img){
  ...
  img.outerHTML = '<span class="' + img.dataset.rclase + '">' + (img.dataset.rtexto || "") + '</span>';
}

/* Marca de reemplazo mientras no exista la foto de producto */
function fotoConRespaldo(id, n, alt, clase){
  return imagenFlexible("img/productos/" + id + "-" + n, alt, clase, "sin-foto", "Foto " + n + " pendiente");
}
...
function montarCabecera(activo){
  ...
  if (nav){
    nav.innerHTML =
      '<a href="index.html?categoria=todas"' +
      (!activo || activo === "todas" ? ' class="activo" aria-current="page"' : '') +
      '>Todas</a>' +
...
function ubicarBuscador(){
  ...
  // Marca invisible que recuerda dónde estaba en la barra
...
/* ==========================================================================
   ...
   ========================================================================== */
/* La letra corresponde al TIPO de medida, no a la posición de la columna.
   Así un chaleco sin mangas conserva A y B sin que se corra la numeración. */
const LETRAS_MEDIDA = {
  "Ancho pecho": "A",
  "Largo": "B",
  "Largo manga": "C",
...
function letraMedida(nombre, indice){
  const l = LETRAS_MEDIDA[nombre] || "ABCDE"[indice] || "";
  return l ? '<span class="letra-medida">' + l + '</span> ' : "";
}

/* Prendas cuya guía de medidas SÍ tiene imagen en img/tallas/.
   Las demás muestran solo la tabla, sin recuadro vacío.
   Para agregar una, sube la imagen a img/tallas/ con el nombre de la clave
   y escribe esa misma clave en esta lista. */
const GUIAS_CON_IMAGEN = [
  "cyclone-u", "cyclone-m",
  "aerofusion-u", "aerofusion-m",
  "sporty-u", "sporty-m",
  "jetline-u", "jetline-m",
  "ultraice-u",
  "business-h", "business-m",
  "fleeceflex-u",
  "skyflow-u",
  "chaleco-fleece-u",
  "softshell-u", "softshell-m"
];
...
function dibujarTallas(mapa, contenedor){
  ...
  function pintar(clave){
    ...
    const conImagen = GUIAS_CON_IMAGEN.indexOf(clave) >= 0;

    cuerpo.innerHTML =
      '<div class="tallas-caja' + (conImagen ? '' : ' tallas-caja-sola') + '">' +
        (conImagen
          ? '<figure class="tallas-esquema" data-guia="' + clave + '">' +
              '<img src="img/tallas/' + clave + '.jpg" alt="Guía de medidas"' +
              ' loading="lazy" decoding="async"' +
              ' onerror="ocultarGuia(this)">' +
              '<figcaption>Toca para ampliar</figcaption>' +
            '</figure>'
          : '') +
        '<div class="tabla-envoltura"><table class="tallas">' +
          '<thead><tr><th scope="col">Talla</th>' +
            t.columnas.map((c, i) => '<th scope="col">' + letraMedida(c, i) + escapar(c) + '</th>').join("") +
          '</tr></thead><tbody>' +
...
/* Si la imagen de la guía no existe, se retira el recuadro en vez de
   dejar un espacio vacío. Prueba .jpg, .webp y .png antes de rendirse. */
function ocultarGuia(img){
  const orden = [".jpg", ".webp", ".png"];
  const actual = (img.getAttribute("src") || "").match(/\.[a-z]+$/);
  const i = actual ? orden.indexOf(actual[0]) : -1;
  if (i >= 0 && i < orden.length - 1){
    img.src = img.getAttribute("src").replace(/\.[a-z]+$/, orden[i + 1]);
    return;
  }
  const fig = img.closest(".tallas-esquema");
  const caja = img.closest(".tallas-caja");
  if (fig) fig.remove();
  if (caja) caja.classList.add("tallas-caja-sola");
}

/* Al tocar la guía de medidas se abre a pantalla completa */
function activarGuiaTallas(contenedor){
  const fig = $("[data-guia]", contenedor);
  if (!fig) return;
  const img = $("img", fig);
  if (!img) return;   // todavía no hay imagen subida

  fig.classList.add("ampliable");
  fig.addEventListener("click", () => {
    const actual = $("img", fig);
    if (actual) Visor.abrir([actual.getAttribute("src")], "Guía de medidas", 0);
  });
}

/* ==========================================================================
   VIDEO DE PROYECTOS
   Arranca solo y en silencio; el visitante decide pausar o activar el sonido.
   ========================================================================== */
function montarVideo(){
  const video = $("#video-proyectos");
  const mando = $("#video-mando");
  if (!video || !mando) return;

  mando.innerHTML =
    '<button type="button" data-video="pausa" aria-label="Pausar el video">' + ICONOS.pausa + '</button>' +
    '<button type="button" data-video="sonido" aria-label="Activar el sonido">' + ICONOS.mudo + '</button>';

  const bPausa  = $('[data-video="pausa"]', mando);
  const bSonido = $('[data-video="sonido"]', mando);

  bPausa.addEventListener("click", () => {
    if (video.paused){
      video.play();
      bPausa.innerHTML = ICONOS.pausa;
      bPausa.setAttribute("aria-label", "Pausar el video");
    } else {
      video.pause();
      bPausa.innerHTML = ICONOS.play;
      bPausa.setAttribute("aria-label", "Reanudar el video");
    }
  });

  bSonido.addEventListener("click", () => {
    video.muted = !video.muted;
    bSonido.innerHTML = video.muted ? ICONOS.mudo : ICONOS.audio;
    bSonido.setAttribute("aria-label", video.muted ? "Activar el sonido" : "Silenciar");
  });

  // Arranca en silencio. El sonido solo se activa si el visitante lo pide.
  video.muted = true;
  video.setAttribute("playsinline", "");

  let pausadoAMano = false;
  bPausa.addEventListener("click", () => { pausadoAMano = !video.paused ? false : true; });

  function reproducir(){
    const intento = video.play();
    if (intento && intento.catch) intento.catch(() => {
      bPausa.innerHTML = ICONOS.play;
      bPausa.setAttribute("aria-label", "Reproducir el video");
    });
  }

  // El video empieza cuando la sección entra en pantalla y se detiene al salir,
  // para no gastar datos ni batería mientras no se está viendo.
  const seccion = video.closest(".video-seccion") || video;
  if ("IntersectionObserver" in window){
    new IntersectionObserver(entradas => {
      entradas.forEach(e => {
        if (e.isIntersecting){
          if (!pausadoAMano) reproducir();
...
/* ==========================================================================
   ...
   Cada bloque aparece con un desplazamiento corto la primera vez que se ve.
...
function activarAnimaciones(){
  ...
  // Las tarjetas del catálogo NO se animan: se repintan con cada filtro
  // y cualquier tropiezo las dejaría invisibles.
  const seleccion = [
    ".seccion-cabeza", ".pilar", ".paso", ".medio", ".faq details",
    ".video-marco", ".video-texto",
    ".manifiesto-intro", ".franja .contenedor"
  ].join(",");
  ...
  function registrar(raiz){
    (raiz || document).querySelectorAll(seleccion).forEach((el, i) => {
      if (el.classList.contains("aparece") || el.classList.contains("visible")) return;
  ...
  registrar(document);
  window.addEventListener("load", () => registrar(document));

  // Red de seguridad: si algo impide que el observador dispare,
  // a los 2,5 segundos todo se muestra igual. Nada puede quedar invisible.
  setTimeout(function(){
...
function montarMarcas(){
  // Portada: rejilla con todas las marcas, sin movimiento
  pintarRejillaMarcas($("#marcas-rejilla"));
  // Ficha de producto: cinta en movimiento
  pintarCinta($("#marcas"), false);
}

/* Rejilla estática con todas las marcas */
function pintarRejillaMarcas(caja){
  if (!caja || !window.MARCAS || !window.MARCAS.length) return;

  caja.innerHTML =
    '<div class="contenedor">' +
      '<div class="seccion-cabeza"><div>' +
        '<p class="rotulo">Clientes</p>' +
        '<h2 class="display">' + escapar(window.MARCAS_TITULO) + '</h2>' +
        '<p class="marcas-pie">' + escapar(window.MARCAS_SUBTITULO) + '</p>' +
      '</div></div>' +
      '<div class="rejilla-marcas">' +
        window.MARCAS.map(nombre =>
          '<div class="marca-celda">' +
            imagenFlexible("img/marcas/" + claveMarca(nombre), nombre, "", "marca-texto", nombre,
                           ["png", "webp", "jpg", "svg"]) +
          '</div>'
        ).join("") +
      '</div>' +
...
function bloqueMedio(item, carpeta){
  return '<figure class="medio">' +
    imagenFlexible("img/" + carpeta + "/" + item.id,
                   item.nombre + " sobre prenda Serranía", "", "medio-sin-foto", "") +
...
/* ==========================================================================
   CIFRAS QUE CUENTAN
   Arrancan en cero y suben hasta su valor cuando entran en pantalla.
   Se activa una sola vez por visita.
   ========================================================================== */
function animarCifras(){
  const cifras = $$(".cifra strong");
  if (!cifras.length) return;

  // Si el visitante pidió menos movimiento, se muestran directo
  if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;

  cifras.forEach(el => {
    const texto = el.textContent.trim();
    const numero = parseFloat(texto.replace(/[^\d.,]/g, "").replace(/\./g, "").replace(",", "."));
    if (!numero || numero < 2) return;   // los valores muy chicos no se animan

    el.dataset.final = texto;
    el.dataset.valor = numero;
    el.textContent = texto.replace(/[\d.,]+/, "0");
  });

  function contar(el){
    const meta   = +el.dataset.valor;
    const modelo = el.dataset.final;
    const inicio = performance.now();
    const duracion = 1400;

    function paso(ahora){
      const t = Math.min((ahora - inicio) / duracion, 1);
      // Arranca rápido y frena al final: se siente más natural que subir parejo
      const suave = 1 - Math.pow(1 - t, 3);
      const actual = Math.round(meta * suave);
      el.textContent = modelo.replace(/[\d.,]+/, actual.toLocaleString("es-CO"));
      if (t < 1) requestAnimationFrame(paso);
      else el.textContent = modelo;
    }
  ...
  const vigia = new IntersectionObserver(entradas => {
    entradas.forEach(e => {
      if (!e.isIntersecting || e.target.dataset.contado) return;
      e.target.dataset.contado = "1";
      contar(e.target);
      vigia.unobserve(e.target);
    });
  }, { threshold: 0.4 });

  cifras.forEach(el => { if (el.dataset.final) vigia.observe(el); });
}
